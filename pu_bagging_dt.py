...
'''
Tune max_smples
1. Hide some positive values from Training set to check how well the model indentify the positives
2. Test the accuracy
'''
...
hidden_size = round(sum(feature)*0.7)
...
bagc = BaggingPuClassifier(DecisionTreeClassifier(), 
                         n_estimators = 1000, 
                         n_jobs = -1, 
                         max_samples = 35000
                        )
...
print('Precision: ', precision_score(y_orig, bagc.predict(data)))
print('Recall: ', recall_score(y_orig, bagc.predict(data)))
print('Accuracy: ', accuracy_score(y_orig, bagc.predict(data)))
...
bagc.fit(data,feature)

# Test set 
eval_f = eval_data.drop(columns= ['language'])
classifier = bagc.fit(data, feature)


eval_data["ebb_eligible"] = bagc.predict(eval_f).astype("int")
